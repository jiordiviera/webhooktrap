...
export interface RollingNumberProps {
  ...
  speed?: number;
  /**
   * Frames to animate over. Defaults to the root composition's
   * `durationInFrames` — override this when nesting inside a
   * `<Sequence>`/`<TransitionSeries.Sequence>` shorter than the full
   * composition, otherwise the count-up paces itself against the wrong
   * (much longer) duration and never visibly finishes.
   */
...
export function RollingNumber({
  ...
  const e = easedProgress(frame, speed, durationInFrames ?? rootDurationInFrames);
